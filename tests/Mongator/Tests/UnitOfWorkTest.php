...
class UnitOfWorkTest extends TestCase
{
    ...
    {
        $unitOfWork = new UnitOfWork(new Mongator($this->metadataFactory, $this->cache));
        ...
        $category3 = $this->mongator->create('Model\Category');
    ...
    public function testCommit()
    {
        ...
        $author1 = $this->mongator->create('Model\Author');
        $author2 = $this->mongator->create('Model\Author');
        $category1 = $this->mongator->create('Model\Category');
        $category2 = $this->mongator->create('Model\Category');
        ...
        $mongator = $this->getMockBuilder('Mongator\Mongator')->disableOriginalConstructor()->getMock();
        $mongator->expects($this->any())->method('getRepository')->will($this->returnCallback($callback));

        $unitOfWork = new UnitOfWork($mongator);
